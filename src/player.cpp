...
void Player::HandleInput(void)
{
    ...
    if (State == PLAYER_ATTACKING || State == PLAYER_DRINKING_POTION)
    {
        ...
        if (ActionTimer <= 0.0f)
        {
            State = PLAYER_IDLE;
            ActionTimer = 0.0f;
            // Jika baru saja menyerang, kembalikan animasi ke IDLE
...
// ================================================================
// Update()
// Handle logic per frame (physics, collision, animation timing).
// Hanya jalan kalau player hidup dan tidak dalam UI mode.
// ================================================================
void Player::Update(void)
{
    // jangan gerak/update kalau mati, inventory/map terbuka
...
{
    // Ambil source rectangle dari spritesheet Knight.png (HEAD Logic)
