...
TEST_CASE("FRAME_SIZE == 32") { CHECK(FRAME_SIZE == 32); }

TEST_CASE("FRAME_GAP == 4") { CHECK(FRAME_GAP == 4); }

TEST_CASE("MAX_TEXTURES == 7") { CHECK(MAX_TEXTURES == 7); }

TEST_CASE("Time::DELTA_TIME == 1.0f/60.0f") {
  CHECK(Time::DELTA_TIME == 1.0f / 60.0f);
}

TEST_CASE("Time::MAX_FRAME == 0.25f") { CHECK(Time::MAX_FRAME == 0.25f); }
...
TEST_CASE("SPAWN_RETRY_LIMIT == 200") { CHECK(SPAWN_RETRY_LIMIT == 200); }
...
TEST_CASE("WG variety iterations == 2") { CHECK(WG_VARIETY_ITERATIONS == 2); }
...
TEST_CASE("CELL_SIZE == 57 (int, 32 * 1.8f truncates)") {
  CHECK(CELL_SIZE == 57);
  CHECK(CELL_SIZE ==
        doctest::Approx(57.6f).epsilon(0.1f)); // 57.6f truncated to int
}
...
TEST_CASE("SAVE_VERSION == 2") { CHECK(SAVE_VERSION == 2); }
